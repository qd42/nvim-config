  ...
  -- Provides advanced syntax highlighting, indentation, and text objects
  {
    ...
    build = function()
      ...
      if vim.fn.has("win32") == 1 then
        vim.notify("TreeSitter installed. Run ':TSInstall lua vim' manually to install parsers.", vim.log.levels.INFO)
        ...
        keys = {
          ...
          s = { -- example of a custom action that toggles the severity
            action = function(view)
              ...
              view:filter({ severity = severity }, { id = "severity", template = "{hl:Title}SEVERITY{hl} {severity}" })
            end,
